class Solution:
    def maxAreaOfIsland(self, grid: List[List[int]]) -> int:
        ...
        # running area for the current island
        area = 0
        ...
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                # if the current tile is land and hasn't been visited, we can add it to the visited set and stack, then set the area
                if grid[i][j] == 1:
                    ...
                    while stack:
                        x, y = stack.pop()
                        # check all 4 surrounding coords, we don't have a new island for each of these, keep adding to visited to show that we've seen them/explored. add to stack as well to continue the bfs process. we should also add to the area.
                        ...
                        if y < len(grid[0]) - 1 and grid[x][y+1] == 1 and (x, y+1) not in visited:
                            ...
                            visited.add((x, y+1))
                            stack.append((x, y+1))
                    # at the end, make sure that we only take the max of the area.
